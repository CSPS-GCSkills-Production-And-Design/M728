...
ADDED IN 0.9036_H
-Re-Randomize questions on .qs-reset-retry (M500/M6 use case) (WILL NOT RE-PICK FROM POOL, ONLY RE-SCRAMBLE QUESTIONS)

ADDED IN 0.9036_G
-Added more info in EventDispatcher/EventRegistrar for activity_completed (activity_successfully_completed:Boolean)

ADDED IN 0.9036_F
-Modified A/BUTTON removing from end of recap template in qsActivityCompleted() to target only *="qs-" classes

ADDED IN 0.9036_E
-Modernize EventDispatcher (qsEventRegistrar)
...
-Fully tests new EventDispatcher (qsEventRegistrar)
...
-.qs-resetretry vs .qs-retry-activity = what the ??? Merge 'em! + MAKE SURE SEMANTIC VALIDATION THROWS MESSAGE (It seems they are not the same after all, one for the intro, one for the conclusion...)
...
function qsBeta(objAcc,paramObj) {
	...
	var lang = paramObj.language || ($("html").attr("lang") || 'fr');
	//SET DEFAULT PATHS IF OVERRIDES ARE UNDEFINED
	var settingsPath = paramObj.settingsPath || 'settings/';
	var settingsFilename = paramObj.settingsFilename || 'qssettings';
	var settingsLabelsFilename = paramObj.settingsLabelsFilename || 'qslabels_';
	...
	this.qsValidateSettingData = function (sData){
		...
		//VALIDATE SETTINGS DATA
	  	...
	  	var validSettingsAttributes = {
	  		settings:["autoTabbing","masterIntTester"],
	  		activity:["activity_type","question_skipping_enabled","tracking_enabled","recap_required","trigger_course_completion","trigger_completion_score","final_feedback_score_threshold","submit_all_at_once","submit_aao_disable_right_anwers","simple_act_disable_mode","reset_retry_enabled","animations_and_transitions","auto_scroll_to_question","force_insert_final_submit"],
	  		exercise:["exercise_type","pool_picks","random_order","feedback_type","randomize_answers","submit_all_at_once"],
	  		question:["question_type","branching_type","question_value"],
	  		type_0:["empty_allowed","min_char_required"],
	...
	*/
	...
	this.settings = qsData.settings;
		...
		this.settings.debugMode = paramObj.debugMode || false;
		this.settings.cheatMode = paramObj.cheatMode || false;
		this.settings.autoStart = paramObj.autoStart || true;
	...
	try{
		...
		if(this.settings.debugMode){
			trackingObj.getData = function(){return $("#qs-tracking-debug").val();};
		}else{
	...
	this.settings.encodeAnswers = (typeof this.settings.encodeAnswers !== 'undefined') ? this.settings.encodeAnswers : true;

	/*WIP*/
	if(this.settings.cheatMode){
		//WARNING FOR THOSE EXPECTING cheatMode TO WORK WITH ENCODED ANSWERS
	...
	//TEST AGAINST THESE AS THEY MAY BREAK EVERYTHING (THEY ARE RESERVED FOR STRING SPLITING) -> "░▒▓╠╣┌┐║♥♦♠♣";"¼½¾Ð¶¢®©ª¤£¦§±þ§Þß¥º·¹²³¨÷|"
	this.settings.illegalCharsList = this.settings.illegalCharsList || (paramObj.illegalCharsList || "¼½¾Ð¶¢®©ª¤£¦§±þ§Þß¥º·¹²³¨÷|");
	// ║ -> ¦	// ┌ -> §	// ┐ -> ¤	// ╠ -> ±	// ╣ -> º	//--------	// ♥ -> ¼	// ♦ -> ½
	//--------
	...
	this.settings.animBaseDuration = this.settings.animBaseDuration || (paramObj.animBaseDuration || 200);
	
	//SECURITY "SEED" VALIDATION INTEGER
	this.settings.masterIntTester = this.settings.masterIntTester || (paramObj.masterIntTester || 33); //KEEP IT OVER 3 AND UNDER 100 PLS! THE HIGHER THE NUMBER THE MORE COMPLEX TO DECODE (FOR END USER SNOOPING AROUND THAT IS!)
	...
	this.qsInit = function (){
		...
		for(i=0;i<$(".qs-elearning-activity").length;i++){
			...
			for(ii=0;ii<$(currElem).find(".qs-exercise").length;ii++){
				...
				inherit_random_answers = eval(($(currElem_ii).attr('data-random-answers')===undefined)?this.objDefVal.exercise.randomize_answers:eval($(currElem_ii).attr('data-random-answers'))==="true");
				...
				this.exercisesArr[this.exercisesArr.length] = {
					...
					"randomize_questions":eval($(currElem_ii).attr('data-random-questions')), //*randomize questions...
...
//$('#'+this.activities[ancestryIDARR[0]].id).find('.qs-submit-activity').attr('data-pass-id-b',this.activities[ancestryIDARR[0]].exercises[this.activities[ancestryIDARR[0]].nb_exercises-1].questions[this.activities[ancestryIDARR[0]].exercises[this.activities[ancestryIDARR[0]].nb_exercises-1].nb_questions-1].id);
	...
	this.qsActivityCompleted = function (elem){
		...
		//DISPATCHER CALL!
		fQsEvDp('activity_completed',{activity_dom_id:this.activities[ancestryIDARR[0]].id,activity_successfully_completed:this.qsComputeActivityScore(ancestryIDARR[0],'perc')>=this.activities[ancestryIDARR[0]].final_feedback_score_threshold});
	...
	this.qsResetActivity = function (activity_id){
		fCl('qsResetActivity('+activity_id+')');
		fCl('-> NOTE THAT RESETING AN ACTIVITY WILL NOT RESET THE RANDOM ORDER OF QUESTIONS! (WORKING ON THIS RIGHT NOW)');
		fCl('-> NOTE THAT RESETING AN ACTIVITY WILL PICK IN THE RANDOM POOL!');
		var i,ii,iii;
		...
		*/
		...
		//RE-RANDOMIZE QUESTIONS (WILL NOT PICK NEW QUESTIONS IN POOL, ONLY RE-SCRAMBLE EXISTING ONES)
		for(reran=0;reran<this.activities[activity_id].exercises.length;reran++){
			//IF NECESSARY
			if(this.activities[activity_id].exercises[reran].randomize_questions){
				var newStackIds = this.qsGetRandomQuestionStack(activity_id,reran);
				//CHECK IF EVERYTHING CHECKS OUT (SAME # OF QUESTIONS IN BOTH STACKS AND MASTER OBJECT)
				if(this.activities[activity_id].exercises[reran].questions.length == newStackIds.length){
					//STRIP MASTER OBJECT OF QUESTIONS BEFORE RE-ORDERING THEM
					var newStackBuffer = this.activities[activity_id].exercises[reran].questions.splice(0,newStackIds.length);
					for(var qLen=0;qLen<newStackIds.length;qLen++){
						//RE-ASSIGN QUESTION FROM BUFFER
						this.activities[activity_id].exercises[reran].questions[qLen] = newStackBuffer[newStackIds[qLen]];
					};
				}else{
					fCe('Number of random questions is not equal to random stack length, please investigate (randomize question order)...');
				}
				//REMOVE REFERENCES
				delete newStackBuffer, newStackIds;
			}
		}
	...
	//EVENT REGISTRAR
	var qsEvRegRegistryObj = {} //Stores callbacks, actions, params to send etc...

	this.qsEventRegistrar = function(eventName,callbackFunction,reqParamsArr){
		fCl("qsEventRegistrar("+eventName+",callbackFunction,reqParamsArr)");
		if(typeof eventName === 'undefined'){
			var helpStr = "Usage:"+
				"\nqsEventRegistrar(event_name,callback_function,required_parameters_array)"+
				"\nevent_name => see below"+
				"\nactivity_started: Invoked when activity starts, params include activity ID (activity_dom_id)"+
				"\nquestion_navigation: Invoked when user changes question, params include activity ID (activity_dom_id), direction of navigation (direction)"+
				"\nquestion_answered: Invoked when user answers and submits a question, params include activity ID (activity_dom_id), correctness of given answer (answer_status) as right/wrong"+
				"\nactivity_completed: Invoked when user completes an activity, params include activity ID (activity_dom_id), pass/fail (activity_successfully_completed)"+
				"\ncallback_function => reference to your function to be called upon event trigger"+
				"\nrequired_parameters_array => information to be passed to your function on event trigger (they are in parenthesis in the definition under 'event_name' or use your own)";
			return helpStr;
		}else{
			var validEvents = ['activity_started','question_navigation','question_answered','activity_completed'];
			var evRegApproved = false;
			//CHECK FOR VALID EVENT NAME
			for(var ri=0; ri<validEvents.length;ri++){
				if(validEvents[ri] === eventName){
					evRegApproved = true;
				};
			}
			//CANCEL IF INVALID 
			...
			//CANCEL IF FUNCTION INVALID
			if(typeof callbackFunction !== 'function'){
				return false;
			}
			//CANCEL IF REQUIRED PARAMS ARE INVALID
			//...
			//REGISTER EVENT
			qsEvRegRegistryObj[eventName] = {'callbackfunction':callbackFunction,'params':reqParamsArr}
			//qsEvRegRegistryObj

//console.dir(qsEvRegRegistryObj)

			return qsEvRegRegistryObj;
		}
	...
	function fQsEvDp(evt,pars){
		fCl('fQsEvDp('+evt+')');
		//CHECK EVENT REGISTAR (TURNED OFF FOR NOW)
		if(typeof qsEvRegRegistryObj[evt] !== 'undefined'){
			//CREATE VALUES HOLDER
			evRequestedStack = ['BA','B'];

//console.log('qsEvRegRegistryObj[evt].params.length: '+qsEvRegRegistryObj[evt].params.length)

			//CHECK FOR DYNAMIC PARAMS & GIVE PROPER VALUES
			for(var evi=0;evi<qsEvRegRegistryObj[evt].params.length;evi++){
				for(var evii in pars){

//console.log('|'+evii+'/'+evi+'   if('+qsEvRegRegistryObj[evt].params[evi]+' == '+evii+') => '+ pars[evii]);

					if(qsEvRegRegistryObj[evt].params[evi] == evii){

//console.log(qsEvRegRegistryObj[evt].params[evi] +' |||'+evii+'||| '+ pars[evii])						

						qsEvRegRegistryObj[evt].params[evi] = pars[evii];
					}else{

//console.log(qsEvRegRegistryObj[evt].params[evi])

					}
				}
			}
			//CALL FUNCTION ALONG WITH REQUIRED PARAMS
			//qsEvRegRegistryObj[evt].callbackfunction.apply(null, qsEvRegRegistryObj[evt].params);
qsEvRegRegistryObj[evt].callbackfunction.apply(this, evRequestedStack);
